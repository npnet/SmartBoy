...
int32 _upgrade_start_download_firmware( const char *ps_url, const char *ps_fwPath){
  /*
    int i4_ret = _upgrade_download_full_ota_package(g_url,UPGRADE_FULL_PKG_SAVE_PATH,info.upgrade_file_size);
    if (i4_ret)
    {
        printf("_upgrade_download_full_ota_package failed\n");
        return i4_ret;
...
int32 Aawant_Get_Dowload_Free_StorageSize(char *path)
{
    ...
    if (i4_ret)
    {
        printf("Aawant_Get_Dowload_Free_StorageSize failed\n");
        return -1;
    }
    avail_size = fs_info.f_bsize * fs_info.f_bavail;
...
int32 Aawant_Wakeup_Data_Flash_Done(DOWNLOAD_PARAM *dl_param, boolean *is_done)
{
    ...
    pthread_mutex_lock(&dl_param->flash_mutex);
    printf("Aawant_Wakeup_Data_Flash_Done\n");
    *is_done = True;
    pthread_cond_signal(&dl_param->flash_cond);
    pthread_mutex_unlock(&dl_param->flash_mutex);
...
int32 Aawant_Get_Download_ImgData(DOWNLOAD_PARAM *dl_param)
{
    ...
    while (img_idx != dl_param->img_num)
    {
        ...
        while (!image_info->is_img_download_done)
        {
            ...
            if (i4_ret)
            {
                printf("Aawant_Download_From_Offset failed\n");
                goto out;
            }
            ...
            if (i4_ret)
            {
                printf("Aawant_Reopen_SaveFile failed\n");
                goto out;
            }
        }
...
int32 Aawwant_Download_OtaPackage(char *url, char *save_path, int32 upg_file_size)
{
    ...
    if (i4_ret)
    {
        printf("%s:%d==>Aawant_DownLoad_init failed\n",__FUNCTION__,__LINE__);
        goto out;
    }
    ...
    if (i4_ret)
    {
        printf("%s:%d==>Aawant_Get_Download_ImgHeadInfo failed\n",__FUNCTION__,__LINE__);
        goto out;
    }
    ...
    if (i4_ret)
    {
        printf("%s:%d==>Aawant_Get_Download_ImgData failed\n",__FUNCTION__,__LINE__);
        goto out;
    }
...
int32 Awant_Get_Download_FullPkgData(DOWNLOAD_PARAM *dl_param)
{
    int32 i4_ret = 0;
    CURL *curl = NULL;
...
int32 Aawant_StartDownLoad(char *g_url,char *save_path,boolean is_full_pkg){
    ...
    if (ret)
    {
        printf("Aawant_Notify_Flash_Done failed\n");
    }
}
